...
const createCourse = async (body) => {
  ...
  try {
    const course = await pgClient.query(`INSERT INTO courses
...
const getCoursesFromUniversity = async (body) => {
  ...
  try {
    const courses = await pgClient.query(`SELECT name FROM courses WHERE university_id = ${id}
...
const deleteCourse = async (body) => {
  ...
  try {
    await pgClient.query(`delete from courses
...
const getCourse = async (body) => {
  ...
  try {
    const course = await pgClient.query(`select name from courses
      where name = '${name}' and  university_id = '${university_id}' and  teacher_id = '${teacher_id}'`);
...
const deleteStudentFromCourse = async (body) => {
  ...
  try {
    await pgClient.query(`delete from students_courses
...
const getStudentFromCourse = async (body) => {
  ...
  try {
    const student = await pgClient.query(`select student_id from students_courses
      where student_id = '${student_id}' and  course_id = '${course_id}'`);
...
const linkStudentToCourse = async (body) => {
  ...
  try {
    const studentCourse = await pgClient.query(`INSERT INTO students_courses (student_id, course_id)
